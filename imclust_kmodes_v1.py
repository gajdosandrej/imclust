...
HELP = f"""
NAME
    imclust_kmodes.py - image clustering demo

USAGE
    imclust_kmodes.py [OPTIONS] DIRECTORY...

DESCRIPTION
    Image clusteuring demo imclust_kmodes_v1.py will cluster images in
...
VERSION
    ...
    imclust_kmodes_v1.py - extended version of imclust.py (by A. Gajdos) 
"""
...
for i in range(len(models)): 
    ...
    for j in range(len(CLUSTERS)): 
        clustering = KModes(n_clusters=int(float(CLUSTERS[j])))
        cl = clustering.fit_predict(vectors[i])
        clusterings[i].append(cl)
        # print(f"clusters: {cl}")

print(f"============================")
print(f"=K-modes clustering - DONE.=")
print(f"============================")
...
for i in range(len(models)): 
    ...
    plt.savefig(output_dataset_path + '\MSC_' + models_names[i] + '_kmodes.png')

    frame.to_csv(output_dataset_path + '\MSC_' + models_names[i] + '_kmodes.txt', index=None, sep='\t', mode='a')
...
for i in range(len(models)): 
    ...
    plt.savefig(output_dataset_path + '\CHS_' + models_names[i] + '_kmodes.png')

    frame.to_csv(output_dataset_path + '\CHS_' + models_names[i] + '_kmodes.txt', index=None, sep='\t', mode='a')
...
for i in range(len(models)): 
    ...
    plt.savefig(output_dataset_path + '\DBS_' + models_names[i] + '_kmodes.png')

    frame.to_csv(output_dataset_path + '\DBS_' + models_names[i] + '_kmodes.txt', index=None, sep='\t', mode='a')
...
for i in range(len(models)): 
    ...
    plt.savefig(output_dataset_path + '\COP_' + models_names[i] + '_kmodes.png')

    frame.to_csv(output_dataset_path + '\COP_' + models_names[i] + '_kmodes.txt', index=None, sep='\t', mode='a')
...
for i in range(len(models)): 
    ...
    plt.savefig(output_dataset_path + '\SDbw_' + models_names[i] + '_kmodes.png')

    frame.to_csv(output_dataset_path + '\SDbw_' + models_names[i] + '_kmodes.txt', index=None, sep='\t', mode='a')
...
for i in range(len(models)):
    for j in range(len(CLUSTERS)):
        ...
        # build the page
        Nazov = f"<h1>algorithm: K-modes, model: " + models_names[i] + ", number of clusters:" + str(CLUSTERS[j]) + "<h1>\n"
        BODY = ""
        ...
        # save html
        # print("write: index_"+ models_names[i] +"_kmodes"+str(CLUSTERS[j])+".html")
        with open(output_dataset_path + "\index_" + models_names[i] + "_kmodes"+str(CLUSTERS[j])+".html","w") as fd:
            fd.write(html)
